...
class ResNet(nn.Module):
    def __init__(self, block, in_planes, num_layers, num_blocks, kernel, skip_kernel, num_classes=10, bias=True):
        ...
        self.path = "./resnet_model.pt"

    def _create_layer(self, block, planes, num_blocks, stride, bias=True):
...
def resnet_model():
    return ResNet(BasicBlock, 32, 4, [2, 2, 2, 2],kernel=(3,1),skip_kernel=(1,0), num_classes=10, bias=True)

if __name__ == "__main__":
    model = ResNet(BasicBlock, 32, 4, [2, 2, 2, 2],kernel=(3,1),skip_kernel=(1,0), num_classes=10, bias=True)
